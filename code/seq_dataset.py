...
class Data(Dataset):
    def __init__(self, n_user, n_item, max_n_node, trn_seq, trn_time, val_seq = None, val_time = None, tst_seq = None, tst_time = None, is_eval = False):
        ...
        # items, num_node, target, session_len = [], [], [], []

        items, times, num_node, target, target_time, session_len = [], [], [], [], [], []
        if val_seq == None:
            for session, t in zip(trn_seq, trn_time):
                for i in range(len(session) - 1):
                    target.append(session[i + 1])
                    items.append(session[:i + 1])
                    times.append(t[:i + 1])
                    target_time.append(t[i + 1])
        elif tst_seq == None:
            for session1, t1, session2, t2 in zip(trn_seq, trn_time, val_seq, val_time):
                for i in range(len(session2)):
                    ...
                    items.append(temp)
                    times.append(temp_t)
                    target_time.append(t2[i])
        else:
            for session1, t1, session2, t2, session3, t3 in zip(trn_seq, trn_time, val_seq, val_time, tst_seq,
                                                                tst_time):
                session = session1 + session2
                t = t1 + t2
                for i in range(len(session3)):
                    target.append(session3[i])
                    target_time.append(t3[i])
        ...
        # self.seq_len = self.items.shape[0]

    def __len__(self):
        return self.seq_len

    def __getitem__(self, index):
        # return self.target[index], self.session_len[index], self.items[index]
        return self.items[index], self.times[index], self.target[index], self.target_time[index]

# def seq_to_temporal_session_graph(seq, times):
#
#     edges = [[seq[i], seq[i + 1]] for i in range(len(seq) - 1)]
#     edge_times = [times[i + 1] for i in range(len(times) - 1)]
#
#     if len(edges) == 0:
#         edges = [0, 0]
#         edge_times = [0.0]
...
#     edge_times = torch.FloatTensor(edge_times)
#     seq = torch.LongTensor(seq)
#
#     return edges, edge_times, seq
#
#
# def collate_fn_factory_temporal(*seq_to_graph_fns):
#     def collate_fn(samples):
#         seqs, times, labels, session_len = zip(*samples)
#         for seq_to_graph in seq_to_graph_fns:
#             local_graphs = list(map(seq_to_graph, seqs, times))
#             num_nodes = [len(graph[0]) for graph in local_graphs]
#             max_num = max(num_nodes)
#             out_seq = torch.vstack([F.pad(graph[2], (0, max_num - len(graph[2])), value=0) for graph in local_graphs])
#             out_seq = out_seq.to(gol.device)
#             out_graph = torch.vstack([graph[0] for graph in local_graphs])
#             out_graph = out_graph.to(gol.device)
#             out_time = torch.vstack([F.pad(graph[1], (0, max_num - len(graph[1])), value=0.0) for graph in local_graphs])
#             out_time = out_time.to(gol.device)
#
#
#         labels = torch.LongTensor(labels).to(gol.device)
#         session_len = torch.LongTensor(session_len).to(gol.device)
#
#         # print(inputs[0].edata)
#         return out_graph, labels, out_seq, out_time, session_len
#
#     return collate_fn
...
def seq_to_temporal_session_graph(seq, times):
    ...
    g.ndata['t']  = torch.tensor(times)[indices]
...
def collate_fn_factory_temporal(*seq_to_graph_fns):
    def collate_fn(samples):
        seqs, times, labels, target_time = zip(*samples)
        # seq = torch.tensor(seqs)
        # time = torch.LongTensor(times)
        max_seq_len = 0
        for index in seqs:
            max_seq_len = max(max_seq_len, len(index))
        inputs = []
        for seq_to_graph in seq_to_graph_fns:
            graphs = list(map(seq_to_graph, seqs, times))
            num_nodes = torch.tensor([graph.number_of_nodes() for graph in graphs], dtype=torch.long)
            embeds_id = torch.vstack([F.pad(torch.LongTensor(s), (0, max_seq_len - len(s)), value=0) for s in seqs])
            times = torch.vstack([F.pad(torch.FloatTensor(t), (0, max_seq_len - len(t)), value=1.0) for t in times])
            bg = dgl.batch(graphs)
            inputs.append(bg)
        labels = torch.LongTensor(labels)
        target_time = torch.FloatTensor(target_time)
        inputs = [x.to(gol.device) for x in inputs]
        # print(inputs[0].edata)
        return inputs, labels.to(gol.device), embeds_id.to(gol.device), times.to(gol.device), num_nodes.to(gol.device), target_time.to(gol.device)

    return collate_fn


def collate_fn_factory_temporal2(*seq_to_graph_fns):
    def collate_fn(samples):
        seqs, times, labels, target_time = zip(*samples)
        # seq = torch.tensor(seqs)
        # time = torch.LongTensor(times)
        ...
        for seq_to_graph in seq_to_graph_fns:
            ...
            max_num = max(num_nodes)
            embeds_id = torch.vstack([F.pad(torch.LongTensor(s), (0, max_seq_len - len(s)), value=0) for s in seqs])
            tensor_times = torch.vstack([F.pad(torch.FloatTensor(t), (0, max_seq_len - len(t)), value=1.0) for t in times])
            bg = dgl.batch(graphs)
            inputs.append(bg)
        labels = torch.LongTensor(labels)
        target_time = torch.FloatTensor(target_time)
        inputs = [x.to(gol.device) for x in inputs]
        # print(inputs[0].edata)
        return inputs, labels.to(gol.device), seqs, times, embeds_id.to(gol.device), tensor_times.to(gol.device), num_nodes.to(gol.device), target_time.to(gol.device)
...
def getDatasets(path='../data', dataset='ml-100k'):
    ...
    processed_path = join(path, dataset, 'ours_processed.pkl')
    if exists(processed_path):
        with open(processed_path, 'rb') as f:
            trn_seq, trn_t, time_piv = pkl.load(f)
        ...
        with open(processed_path, 'wb') as f:
            pkl.dump((trn_seq, trn_t, time_piv), f, pkl.HIGHEST_PROTOCOL)
    ...
    for idx, session in enumerate(trn_seq):
        for i in range(len(session) - 1):
            whole_graph.append([session[i], session[i + 1]])
            graph_time.append(trn_t[idx][i + 1])
    whole_graph = torch.LongTensor(whole_graph).T

    return n_item, max_seq_len, tr_set, va_set, te_set, time_piv, whole_graph, graph_time
